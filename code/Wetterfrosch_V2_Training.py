...
"""
wetterfrosch_v2 = keras.Sequential()
wetterfrosch_v2.add(keras.layers.GRU(units=16,
                                     activation='relu',
                                     dropout=0.1,
                                     recurrent_dropout=0.4,
                                     return_sequences=True,
                                     input_shape=(zeitraum_vergangenheit, stuendlich.shape[-1])))
wetterfrosch_v2.add(keras.layers.GRU(units=32,
                                     activation='relu',
                                     dropout=0.1,
                                     recurrent_dropout=0.4,
                                     return_sequences=True))
wetterfrosch_v2.add(keras.layers.GRU(units=64,
                                     activation='relu',
                                     dropout=0.1,
                                     recurrent_dropout=0.4,
                                     return_sequences=False))
wetterfrosch_v2.add(keras.layers.Dense(1))
wetterfrosch_v2.compile(optimizer=keras.optimizers.RMSprop(),
                        loss='mae')

utils.plot_model(a=wetterfrosch_v2,
                 to_file="wetterfrosch_v2.png",
                 show_shapes=True,
                 show_layer_names=True)

history = wetterfrosch_v2.fit(x=training_x,
                              y=training_y,
                              epochs=120,
                              batch_size=32,
...
"""
evaluierung = wetterfrosch_v2.evaluate(x=test_x, y=test_y)
print evaluierung
...
"""
wetterfrosch_v2.save('../keras_models/wetterfrosch_v2.h5')
